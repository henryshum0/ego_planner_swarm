...
public:
    ...
    const std::vector<int> & rawOccupiedAddresses() const {return active_occupied_;}
    const std::vector<int> & inflatedObstacleAddresses() const {return active_inflated_;}
    Eigen::Vector3d indexToPosition(const Eigen::Vector3i & index) const;
    Eigen::Vector3d addressToPosition(std::size_t address) const;
...
private:
    ...
    // Inflation generated by observed obstacles only. The virtual ceiling is
    // deliberately kept out of this list so it remains collision-active
    // without adding an entire dense plane to visualization messages.
    std::vector < int > active_inflated_;
    std::vector < int > inflated_slot_;
    ...
    void addActiveInflated(std::size_t address);
    void removeActiveInflated(std::size_t address);
